...
export default function InvestorBookAppointmentPage() {
  const session = getAuthSession()
  ...
  useEffect(() => {
    const fullName = [basicInfo?.first_name, basicInfo?.last_name].filter(Boolean).join(' ').trim()
    if (!fullName) {
      return
    }

    setForm((previous) => (
      previous.names ? previous : { ...previous, names: fullName }
    ))
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      ...
      try {
        const userRes = await fetch(buildApiUrl(`/investors/${encodeURIComponent(session.email)}`), {
          headers: authHeader(),
        })
        ...
        if (!userRes.ok) {
          throw new Error(
            typeof userPayload === 'object' && userPayload && 'message' in userPayload && typeof (userPayload as ApiPayload).message === 'string'
              ? (userPayload as ApiPayload).message as string
              : 'Failed to fetch investor profile.',
          )
        }

        const resolvedUserId = toNumber((userPayload as { users_id?: unknown })?.users_id)
        if (!resolvedUserId) {
          throw new Error('Invalid investor user id returned by the server.')
        }

        const basicRes = await fetch(buildApiUrl(`/investors/select_basic/${resolvedUserId}`), {
          headers: authHeader(),
        })
        const basicPayload = await parseResponseBody(basicRes)
        ...
        setForm((previous) => ({
          ...previous,
          email: previous.email || nextBasicInfo?.email || session.email,
          phone: previous.phone || nextBasicInfo?.telephone || '',
          names: previous.names || [nextBasicInfo?.first_name, nextBasicInfo?.last_name].filter(Boolean).join(' ').trim(),
        }))
      } catch {
        if (!isMounted) {
          return
    ...
    <InvestorLayout>
      <section style={investorCardStyle}>
        ...
        <div style={styles.headerBlock}>
          <p style={styles.kicker}>Appointment Booking</p>
          <h2 style={styles.title}>Book an Appointment</h2>
          <p style={styles.description}>
            Reserve a meeting with the Finverra team and view available slots before submitting your request.
          </p>
          <p style={styles.profileHint}>
            {basicInfo?.first_name || basicInfo?.last_name
              ? `Prefilled from your profile: ${[basicInfo.first_name, basicInfo.last_name].filter(Boolean).join(' ').trim()}`
              : 'Your profile name will be used here when available.'}
          </p>
        </div>
          ...
          <section style={styles.panel}>
            <div style={styles.panelHeader}>
              <div>
                ...
                <h3 style={styles.panelTitle}>Request a meeting</h3>
              </div>
            </div>
          ...
          <section style={styles.panel} ref={availabilityRef}>
            <div style={styles.panelHeader}>
              <div>
                ...
                <h3 style={styles.panelTitle}>Available slots</h3>
              </div>
            </div>
            ...
            <div style={styles.noteBox}>
              ...
              <p style={styles.noteText}>
                Your email is prefilled from your investor account when available.
              </p>
            </div>
          </section>
        </div>
...
const styles: Record<string, CSSProperties> = {
  ...
  title: { margin: '6px 0 8px', fontSize: 26, fontWeight: 700, color: '#023341' },
  description: { margin: 0, maxWidth: 680, color: '#023341', opacity: 0.6, lineHeight: 1.6, fontSize: 14 },
  profileHint: { margin: '8px 0 0', fontSize: 12, color: '#023341', opacity: 0.48 },
