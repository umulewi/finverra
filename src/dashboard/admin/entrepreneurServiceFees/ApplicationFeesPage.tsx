...
export default function ApplicationFeesPage() {
  ...
  const [form, setForm] = useState<FormState>({ Investment: '', fees: '' })
  ...
  const loadFees = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await fetch(buildApiUrl('/admin/e_application_fees'), {
        headers: authHeader,
      })
      if (!response.ok) throw new Error(`Error: ${response.status}`)
      const data = await response.json()
      setFees(data.e_application_fees || [])
    } catch (err) {
  ...
  const openCreate = () => {
    setEditingId(null)
    setForm({ Investment: '', fees: '' })
    setFormError(null)
    setShowForm(true)
  }

  const openEdit = (fee: ApplicationFee) => {
    setEditingId(fee.id)
    setForm({ Investment: fee.Investment, fees: fee.fees })
    setFormError(null)
    setShowForm(true)
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.Investment || !form.fees) {
      setFormError('Investment and fees are required')
      return
    }
    ...
    try {
      ...
      const url = editingId
        ? buildApiUrl(`/admin/e_application_fees/${editingId}`)
        : buildApiUrl('/admin/e_application_fees')
  ...
  const handleDelete = async (id: number) => {
    try {
      const response = await fetch(buildApiUrl(`/admin/e_application_fees/${id}`), {
        method: 'DELETE',
        headers: authHeader,
      })
      if (!response.ok) throw new Error(`Error: ${response.status}`)
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h2 style={styles.title}>Application Fees</h2>
          ...
          {fees.map(fee => (
            <div key={fee.id} style={styles.card}>
              <div style={styles.cardHeader}>
                <h3 style={styles.cardTitle}>{fee.Investment}</h3>
              ...
              <p style={styles.cardInfo}>Fees: {fee.fees}</p>
            </div>
          ))}
        </div>
      )}
          ...
          <div style={styles.modalContent} onClick={e => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              <h3 style={styles.modalTitle}>
                {editingId ? 'Edit Application Fee' : 'Create Application Fee'}
              </h3>
            ...
            <form onSubmit={handleSubmit} style={styles.form}>
              ...
              <div style={styles.formGroup}>
                <label style={styles.label}>Fees Amount *</label>
                <input
                  type="text"
                  value={form.fees}
