...
export default function BusinessMentorshipPage() {
  const [fees, setFees] = useState<MentorshipFee[]>([])
  ...
  const loadFees = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await fetch(buildApiUrl('/admin/e_business_mentorship'), {
        headers: authHeader,
      })
      if (!response.ok) throw new Error(`Error: ${response.status}`)
      const data = await response.json()
      setFees(data.e_business_mentorship || [])
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load mentorship fees')
    } finally {
      setLoading(false)
    }
  }
  ...
  const openEdit = (fee: MentorshipFee) => {
    setEditingId(fee.id)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingId
        ? buildApiUrl(`/admin/e_business_mentorship/${editingId}`)
        : buildApiUrl('/admin/e_business_mentorship')
  ...
  const handleDelete = async (id: number) => {
    try {
      const response = await fetch(buildApiUrl(`/admin/e_business_mentorship/${id}`), {
        method: 'DELETE',
        headers: authHeader,
      })
      if (!response.ok) throw new Error(`Error: ${response.status}`)
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h2 style={styles.title}>Business Mentorship</h2>
      ...
      {loading && <p style={styles.loadingText}>Loading mentorship programs...</p>}

      {!loading && fees.length === 0 && <p style={styles.emptyText}>No programs found</p>}
          ...
          <div style={styles.modalContent} onClick={e => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              <h3 style={styles.modalTitle}>
                {editingId ? 'Edit Business Mentorship' : 'Create Business Mentorship'}
              </h3>
