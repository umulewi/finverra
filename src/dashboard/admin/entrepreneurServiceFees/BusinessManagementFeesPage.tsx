...
interface BusinessManagementFee {
  id: number
  duration: string
  amount: string
}
...
export default function BusinessManagementFeesPage() {
  const [fees, setFees] = useState<BusinessManagementFee[]>([])
  ...
  const loadFees = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await fetch(buildApiUrl('/admin/e_business_management_fees'), {
        headers: authHeader,
      })
      if (!response.ok) throw new Error(`Error: ${response.status}`)
      const data = await response.json()
      setFees(data.e_business_management_fees || [])
    } catch (err) {
  ...
  const openEdit = (fee: BusinessManagementFee) => {
    setEditingId(fee.id)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingId
        ? buildApiUrl(`/admin/e_business_management_fees/${editingId}`)
        : buildApiUrl('/admin/e_business_management_fees')
  ...
  const handleDelete = async (id: number) => {
    try {
      const response = await fetch(buildApiUrl(`/admin/e_business_management_fees/${id}`), {
        method: 'DELETE',
        headers: authHeader,
      })
      if (!response.ok) throw new Error(`Error: ${response.status}`)
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h2 style={styles.title}>Business Management Fees</h2>
          ...
          <div style={styles.modalContent} onClick={e => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              <h3 style={styles.modalTitle}>
                {editingId ? 'Edit Business Management Fee' : 'Create Business Management Fee'}
              </h3>
