...
export default function WholeBusinessManagementPage() {
  ...
  const loadFees = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await fetch(buildApiUrl('/admin/i_whole_business_management'), { headers: authHeader })
      if (!response.ok) throw new Error(`Error: ${response.status}`)
      const data = await response.json()
      setFees(data.i_whole_business_management || [])
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingId
        ? buildApiUrl(`/admin/i_whole_business_management/${editingId}`)
        : buildApiUrl('/admin/i_whole_business_management')
  ...
  const handleDelete = async (id: number) => {
    try {
      const response = await fetch(buildApiUrl(`/admin/i_whole_business_management/${id}`), {
        method: 'DELETE',
        headers: authHeader,
      })
      if (!response.ok) throw new Error(`Error: ${response.status}`)
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h2 style={styles.title}>Whole Business Management</h2>
      ...
      {loading && <p style={styles.loadingText}>Loading programs...</p>}
        ...
        <div style={styles.modalOverlay} onClick={closeForm}>
          <div style={styles.modalContent} onClick={e => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              <h3 style={styles.modalTitle}>{editingId ? 'Edit Program' : 'Create Program'}</h3>
