...
export default function ApplicationFeesPage() {
  ...
  const loadFees = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await fetch(buildApiUrl('/admin/i_application_fees'), { headers: authHeader })
      if (!response.ok) throw new Error(`Error: ${response.status}`)
      const data = await response.json()
      setFees(data.i_application_fees || [])
    } catch (err) {
  ...
  const openEdit = (fee: ApplicationFee) => {
    setEditingId(fee.id)
    setForm({ details: fee.details, amount: fee.amount })
    setFormError(null)
    setShowForm(true)
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingId
        ? buildApiUrl(`/admin/i_application_fees/${editingId}`)
        : buildApiUrl('/admin/i_application_fees')
  ...
  const handleDelete = async (id: number) => {
    try {
      const response = await fetch(buildApiUrl(`/admin/i_application_fees/${id}`), {
        method: 'DELETE',
        headers: authHeader,
      })
      if (!response.ok) throw new Error(`Error: ${response.status}`)
        ...
        <div style={styles.modalOverlay} onClick={closeForm}>
          <div style={styles.modalContent} onClick={e => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              <h3 style={styles.modalTitle}>{editingId ? 'Edit Application Fee' : 'Create Application Fee'}</h3>
