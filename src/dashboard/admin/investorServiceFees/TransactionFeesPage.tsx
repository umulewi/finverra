...
export default function TransactionFeesPage() {
  ...
  const [form, setForm] = useState<FormState>({ details: '', percentage: '' })
  ...
  const loadFees = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await fetch(buildApiUrl('/admin/i_transaction_fees'), { headers: authHeader })
      if (!response.ok) throw new Error(`Error: ${response.status}`)
      const data = await response.json()
      setFees(data.i_transaction_fees || [])
    } catch (err) {
  ...
  const openEdit = (fee: FeeItem) => {
    setEditingId(fee.id)
    setForm({ details: fee.details, percentage: fee.percentage })
    setFormError(null)
    setShowForm(true)
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.details || !form.percentage) {
      setFormError('Details and percentage are required')
      return
    }
    ...
    try {
      ...
      const url = editingId
        ? buildApiUrl(`/admin/i_transaction_fees/${editingId}`)
        : buildApiUrl('/admin/i_transaction_fees')
  ...
  const handleDelete = async (id: number) => {
    try {
      const response = await fetch(buildApiUrl(`/admin/i_transaction_fees/${id}`), {
        method: 'DELETE',
        headers: authHeader,
      })
      if (!response.ok) throw new Error(`Error: ${response.status}`)
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h2 style={styles.title}>Transaction Fees</h2>
          ...
          {fees.map(fee => (
            <div key={fee.id} style={styles.card}>
              <div style={styles.cardHeader}>
                <h3 style={styles.cardTitle}>{fee.details}</h3>
              ...
              <p style={styles.cardInfo}>Percentage: {fee.percentage}</p>
            </div>
          ))}
        </div>
      )}
        ...
        <div style={styles.modalOverlay} onClick={closeForm}>
          <div style={styles.modalContent} onClick={e => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              <h3 style={styles.modalTitle}>{editingId ? 'Edit Transaction Fee' : 'Create Transaction Fee'}</h3>
